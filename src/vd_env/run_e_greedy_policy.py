...
def run_e_greedy_policy(environment, exploration=10, max_term_reward=30, epsilon=.1,
                        deterministic=True, is_action_based=False):
    ...
    while environment.is_has_next_term():
        while True:
            if is_action_based:
                action = e_greedy_action_based_policy(environment, state,explore_steps=exploration,
...
if __name__ == '__main__':
    ...
    env, rewards_det = run_e_greedy_policy(env, exploration=15, max_term_reward=max_reward,
                                           deterministic=True, is_action_based=True)
    print('Deterministic: ---------------------->')
    env.render()
    print('\n\n')

    env.reset_soft()
    env, rewards_non_det = run_e_greedy_policy(env, exploration=exploration, max_term_reward=max_reward,
                                               deterministic=False, is_action_based=True)

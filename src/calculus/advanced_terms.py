...
def ite_term():
    ...
    return Lambda(c, Lambda(x, Lambda(y, App(c_, App(x_, y_)))))


def not_term():
    a = Var()
    a_ = Atom(a)
    return Lambda(a, App(App(App(ite_term(), a_), false_term()), true_term()))
...
def or_term():
    a, b = Var(), Var()
    a_, b_ = Atom(a), Atom(b)
    return Lambda(a, Lambda(b, App(ite_term(), App(a_, App(a_, b_)))))
...
def succ_term():
    ...
    return Lambda(n, Lambda(x, Lambda(y, App(x_, App(App(n_, x_), y_)))))


def iszero_term():
    x, n = Var(), Var()
    n_ = Atom(n)
    return Lambda(n, App(n_, App(Lambda(x, false_term()), true_term())))


def pair_term():
    x, y, p = Var(), Var(), Var()
    x_, y_, p_ = Atom(x), Atom(y), Atom(p)
    return Lambda(x, Lambda(y, Lambda(p, App(p_, App(x_, y_)))))
...
def sinc_term():
    ...
    p_ = Atom(p)
    return Lambda(p, App(
        App(pair_term(), App(second_term(), p_)),
...
def pred_term():
    n = Var()
    n_ = Atom(n)
    return Lambda(n, App(first_term(), App(n_, App(sinc_term(), App(pair_term(), App(n0_term(), n0_term()))))))


def substr_term():
    n, m = Var(), Var()
    n_, m_ = Atom(n), Atom(m)
    return Lambda(n, Lambda(m, App(m_, App(pred_term(), n_))))


def le_term():
    n, m = Var(), Var()
    n_, m_ = Atom(n), Atom(m)
    return Lambda(n, Lambda(m, App(iszero_term(), App(substr_term(), App(n_, m_)))))


def eq_term():
    n, m = Var(), Var()
    n_, m_ = Atom(n), Atom(m)
    return Lambda(n, Lambda(m, App(and_term(), App(
        App(le_term(), App(n_, m_)),
        App(le_term(), App(m_, n_))
    ))))
...
def gcd_term():
    ...
    ite_eq_ite = App(App(App(ite_term(), eq_n_m), n), ite_le_sub_sub)

...
class GenTermsThread(Thread):
    def __init__(
        ...
        super().__init__()
        self.unfiltered_terms = gen_lambda_terms(
    ...
    def run(self):
        ...
        if self.mode in ["all", "gen"]:
            self.gen_terms, self.gen_stepsLO = gen_filtered_lambda_terms(
                count_terms=self.count_terms,
        ...
        if self.mode in ["all", "RI"]:
            ...
            self.gen_stepsRI = [
                term.normalize(RightmostInnermostStrategy())[1]
        ...
        if self.mode in ["all", "Rand"]:
            ...
            self.gen_stepsRand = [
                sum(
                    [
                        term.normalize(RandomStrategy())[1]
                        for i in range(self.random_average_count)
                    ]
                )
...
class GenTermsThreadV2(Thread):
    def __init__(
        self, count_terms=100, random_average_count=20, thread_name="", mode="all"
    ):
        super().__init__()

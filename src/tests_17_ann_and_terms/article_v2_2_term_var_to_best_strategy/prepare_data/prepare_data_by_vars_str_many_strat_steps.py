...
def gen_norm_data(terms_list, strategies_list):
    ...
    for term in tqdm(terms_list):
        ...
        while norm_term:
            list_steps_strategy = []
            term_red_step += 1
            vars_terms_.append(norm_term.funky_str())
    ...
    return vars_terms_, steps_strategies_
...
for res_ in list_res_OO:
    ...
    for inx_ in range(len(st)):
        vars_terms.append(st[inx_])
        steps_lo.append(ss[inx_][0])
        steps_ri.append(ss[inx_][1])

df = pd.DataFrame({"LO_steps_num": steps_lo, "RI_steps_num": steps_ri, "vars_terms": vars_terms})
print(len(df))
df = df.drop_duplicates(subset="vars_terms")
print(len(df))

# df.to_csv(f"./data_steps/steps_vars_term_str.csv", index=False)
df.to_csv(f"./data_steps/steps_vars_term_str_train_v1_{from_inx}_{to_inx - 1}.csv", index=False)
